...
import { ChatHistory, type ChatHistoryItem } from "@/components/chat-history";
...
export function WorkspaceSidebar({
  ...
  return (
    ...
    >
      ...
      <div className="flex min-h-0 flex-1 flex-col pb-[188px]">
        {isExpanded && (
          <ChatHistory
            initialHasMore={chatHistoryHasMore}
      ...
      <div className="absolute inset-x-2.5 bottom-3">
        ...
        <nav className="space-y-1" aria-label="School actions">
          {schoolItems.map(({ href, icon: Icon, label }) => {
            const className = cn(
                "flex h-10 w-full items-center overflow-hidden rounded-xl text-left text-sm leading-5 text-[#5f6862] transition hover:bg-[#e4e2dc] hover:text-[#313a35]",
